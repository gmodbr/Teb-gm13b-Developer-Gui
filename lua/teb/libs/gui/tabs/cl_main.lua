function GM13Panel:FillMainTab(MainTab)
   ...
   --[[ Export
   local Export_Button = vgui.Create("DButton")
   Export_Button:SetText("Print Memorys")
   Export_Button:SetMouseInputEnabled(true)
   Export_Button.DoClick = function ()
	   print(GM13.Event.Memory:Get("coneLevel"))
   end
   Export_Button:Dock(TOP)
   Export_Button:DockMargin(0, 0, 0, 5)
   Export_Button:SizeToContents()
   Main_Tab_List:AddItem(Export_Button)--]]

   --[[ sethlth
   local sethlth_Button = vgui.Create("DButton")
   ...
   sethlth_Button:SizeToContents()
   Main_Tab_List:AddItem(sethlth_Button)--]]
 end